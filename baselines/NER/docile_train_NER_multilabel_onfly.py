...
import torch
import torchmetrics
from data_collator import (  # MyDataCollatorForTokenClassification,
    MyMLDataCollatorForTokenClassification,
    MyMLDataCollatorForTokenClassification2,
)
from datasets import Array2D, ClassLabel
from datasets import Dataset as ArrowDataset
from datasets import Features, Sequence, Value

# from datasets.utils.logging import set_verbosity_error, set_verbosity_warning, set_verbosity_info, set_verbosity, disable_progress_bar
...
def tag_fields_with_entities(fields, unique_entities=[]):
    ...
    for token in fields:
        # <HACK> - for synthetic data, remove the non-existent fieldtypes (namely "tax_detail_code", "document_ref_num", "delivery_node_id")
        non_existent_fieldtypes = np.array([ft not in entity_map for ft in token.fieldtype])
        for i, flag in enumerate(non_existent_fieldtypes):
            if flag:
                token.fieldtype.pop(i)
        if not token.fieldtype:
            continue
        # </HACK>
...
class NERDataMaker:
    ...
    def as_hf_dataset(self, _tokenizer, _tag_everything=False, _stride=0):
        from datasets import Array2D, ClassLabel
        from datasets import Dataset as ArrowDataset
        from datasets import Features, Sequence, Value
        ...
        tag_everything = _tag_everything
        stride = _stride
...
def get_data_from_document(document, overlap_thr):
    data, metadata = [], []
...
def get_data_from_docile(split, docile_path, overlap_thr=0.5):
    ...
    for document in tqdm(dataset):
        ...
        #     )
        #     data.append(sorted_fields)
        doc_data, doc_metadata = get_data_from_document(document, overlap_thr=overlap_thr)
...
class DataLoaderWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        # )
        # return tokenized_inputs
        # tokenized_inputs = tokenize_and_align_labels_unbatched(data)

        doc_data, doc_metadata = get_data_from_document(document, overlap_thr=0.5)
        ids, ner_tags, tokens, bboxes = [], [], [], []
        # data = {}
        for i, page_data in enumerate(doc_data):
            tokens_with_entities = tag_fields_with_entities(page_data, self.unique_entities)
            if tokens_with_entities:
                # ids, ner_tags, tokens, infos, bboxes = [], [], [], [], []
                pt_tokens, pt_tags, pt_info = list(zip(*tokens_with_entities))
                ids.append(i)
                # ner_tags.append(tuple([make_labels(x, len(self.unique_entities)) for x in pt_tags]))
                ner_tags.append(
                    tuple(
                        [
                            make_labels(
                                [self.unique_entities.index(e) for e in x],
                                len(self.unique_entities),
                            )
                            for x in pt_tags
                        ]
                    )
        ...
        #         ids.append(i)
        #         # ner_tags.append(tuple([make_labels(x, len(self.unique_entities)) for x in pt_tags]))
        #         ner_tags.extend([make_labels([self.unique_entities.index(e) for e in x], len(self.unique_entities)) for x in pt_tags])
        #         tokens.extend(list(pt_tokens))
        #         # infos.extend(list(pt_info))
        #         bboxes.extend(
        #             [np.array([d[0][0], d[0][1], d[0][2], d[0][3]], dtype=np.int32) for d in pt_info]
        #         )
        # data = {
...
if __name__ == "__main__":
    ...
    if args.on_fly_dataset:
        data_collator = MyMLDataCollatorForTokenClassification2(
    ...
    if args.on_fly_dataset:
        stride = args.stride
        tag_everything = args.tag_everything
        tokenizer = tokenizer

        train_dataset = DataLoaderWrapper(
            dataset=Dataset(
                split_name=args.dataset_split,
                dataset_path=args.docile_path,
