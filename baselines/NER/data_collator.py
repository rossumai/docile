...
@dataclass
class MyDataCollatorForTokenClassification(DataCollatorMixin):
    """
...
@dataclass
class MyMLDataCollatorForTokenClassification2(DataCollatorMixin):
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def torch_call(self, features):
        ...
        for feat in features:
            for i in range(len(feat["input_ids"])):
                ...
                if "bboxes" in feat:
                    mod_feat["bboxes"] = feat["bboxes"][i][0]
                features_mod.append(mod_feat)
