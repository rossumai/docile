...
# import evaluate
from data_collator import (  # MyDataCollatorForTokenClassification,; MyLayoutLMv3MLDataCollatorForTokenClassification,; MyMLDataCollatorForTokenClassification,
    MyLayoutLMv3MLDataCollatorForTokenClassification2,
)
from datasets import Array2D, Array3D, ClassLabel
...
class NERDataMaker:
    ...
    def as_hf_dataset(self, _processor, _tag_everything=False, _stride=0):
        from datasets import Array2D, Array3D, Array4D
        from datasets import Dataset as ArrowDataset
        from datasets import Features, Sequence, Value

        global tag_everythin
        global stride
        global processor
...
def get_data_from_document(document, overlap_thr=0.5):
    data, metadata = [], []
...
class DataLoaderWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        ids, ner_tags, tokens, bboxes, images = [], [], [], [], []
        for i, (page_data, meta) in enumerate(zip(doc_data, doc_metadata)):
            tokens_with_entities = tag_fields_with_entities(page_data, self.unique_entities)
            if tokens_with_entities:
...
if __name__ == "__main__":
    ...
    # )
    data_collator = MyLayoutLMv3MLDataCollatorForTokenClassification2(
        processor=processor,
        max_length=512,
        padding="longest",
    )
    ...
    if args.on_fly_dataset:
        ...
        if args.arrow_format:
            ...
            if args.store_preprocessed:
                ...
                store_metadata(
                    args.store_preprocessed / args.dataset_name / "val_multilabel_metadata_withImgs.json",
                    val_metadata,
                )
