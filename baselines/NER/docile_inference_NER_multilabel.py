...
if __name__ == "__main__":
    ...
    for document in tqdm(dataset):
        ...
        for page in range(document.page_count):
            ...
            tokenized_inputs["bboxes"] = torch.Tensor(np.array(bboxes2, dtype=np.int32)).long().to(device)
            ...
            for b_i in range(num_batch):
                if b_i > 0:
                    ...
                    end_i = np.where(np.array(tokenized_inputs[b_i].tokens) == tokenizer.sep_token)[0][0]
                else:
                    start_i = 1
                    end_i = -1
