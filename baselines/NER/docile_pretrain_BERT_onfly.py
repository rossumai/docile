...
class ScriptArguments:
    ...
    re_order_ocr_boxes: Optional[bool] = field(default=False, metadata={"help": "Flag if the OCR boxes should be re-ordered."})
    warmup_steps: Optional[int] = field(default=5000, metadata={"help": "Warmup steps."})
...
def run_mlm():
    ...
    tokenizer = AutoTokenizer.from_pretrained(script_args.model_config_id, add_prefix_space=True if script_args.model_config_id=="roberta-base" else False)

    dataset = Dataset.from_file(script_args.split, script_args.docile_path)
